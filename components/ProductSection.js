...
export function ProductCard({ title, creator, price, oldPrice, imageSrc, onAddToCart, onBuyNow }) {
  return (
    <div className=" relative rounded-3xl overflow-hidden bg-black/10 backdrop-blur-md border border-white p-4 flex gap-1">
      ...
      <div className="flex-shrink-0 flex flex-col justify-between  w-2/5">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-green-400 font-bold text-sm sm:text-base">₹{price}</span>
          </div>
        </div>
...
export default function ProductSection({ products = [] }) {
  ...
  return (
    <section className=" border-2 border-blue-500  relative w-full py-20 px-10 overflow-hidden">
      {/* NEW ARRIVAL Vertical Text */}
      <div className="absolute left-12 top-0 bottom-0 flex items-center z-10">
        <div className="rotate-180 text-white/80 tracking-widest text-4xl md:text-6xl font-bold" style={{ writingMode: 'vertical-rl' }}>
          NEW ARRIVAL...
        </div>
      </div>
      
      {/* Just dropped header */}
      <div className="relative z-10 flex items-center mb-8 ml-16 md:ml-24">
        <span className="text-red-500 mr-2">✗</span>
        <h2 className="text-3xl md:text-4xl font-schoolbell text-yellow-300">Just dropped...</h2>
      </div>
      
      {/* Products Grid with limited visibility */}
      <div className="relative z-10 grid grid-cols-1 rounded-3xl  bg-black/40 md:grid-cols-2 font-glacial gap-4 ml-10 md:ml-24 p-8 px-15 ">
